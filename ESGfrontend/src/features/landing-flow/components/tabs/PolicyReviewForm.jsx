...
const PolicyReviewForm = ({ companyId }) => {
    const dispatch = useDispatch();
    const questions = useSelector(state => selectQuestionsByTab(state, 'policy_review'));
    const loading = useSelector(selectQuestionsLoading);
    const error = useSelector(selectQuestionsError);
    const hasUnsavedChanges = useSelector(selectHasUnsavedChanges);

    useEffect(() => {
        dispatch(fetchQuestionsByTab('policy_review'));
    }, [dispatch]);
    ...
    return (
        <div className="p-6">
            <div className="mb-6">
                <h2 className="text-2xl font-bold mb-4">Policy Review</h2>
                {hasUnsavedChanges && (
                    <ValidationMessage
